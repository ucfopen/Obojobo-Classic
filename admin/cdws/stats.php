...
$stats = array();
   ...
   $stats[] = array ('name' => '30_InstanceIndividualViewsByDay', 'value'=>"SELECT DATE_FORMAT(FROM_UNIXTIME(T.".cfg_obo_Track::TIME."), '%m-%d-%Y') AS DATE, COUNT(T.".cfg_obo_Track::TIME.") AS VISITS, CONCAT(L.title, ' v.', L.".cfg_obo_LO::VER.") AS MASTER FROM ".cfg_obo_Track::TABLE." AS T, ".cfg_obo_Instance::TABLE." AS I, ".cfg_obo_LO::TABLE." AS L WHERE T.".cfg_obo_Track::TYPE." = 'nm_los_tracking_Visited' AND T.".cfg_obo_Instance::ID." = I.".cfg_obo_Instance::ID." AND L.".cfg_obo_LO::ID." = I.".cfg_obo_LO::ID." AND T.".cfg_obo_Track::TIME." > 1214193600 GROUP BY DATE, MASTER ORDER BY T.".cfg_obo_Track::TIME.", MASTER");
   ...
   $stats[] = array ('name' => '35_NewFacultyUsersByHourOfDay', 'value'=>"SELECT DATE_FORMAT(FROM_UNIXTIME(".cfg_core_User::CREATED_TIME."), '%H:00') AS DATE, COUNT(".cfg_core_User::ID.") AS COUNT FROM ".cfg_core_User::TABLE." WHERE ".cfg_core_User::ID." IN (SELECT ".cfg_core_User::ID." FROM ".cfg_obo_Role::MAP_USER_TABLE." WHERE ".cfg_obo_Role::ID." IN (SELECT ".cfg_obo_Role::ID." from ".cfg_obo_Role::TABLE." WHERE ".cfg_obo_Role::ROLE." = '".cfg_obo_Role::CONTENT_CREATOR."' OR ".cfg_obo_Role::ROLE." = '".cfg_obo_Role::LIBRARY_USER."')) AND     ".cfg_core_User::CREATED_TIME." > 1214193600 GROUP BY DATE ORDER BY ".cfg_core_User::CREATED_TIME."");
...
$startTime = 1262325661;
$endTime = time();
$loIDs = implode(',', array(7841, 7820, 7819, 7814, 7802, 7801, 7792, 7711));

$stats[] = array ('name' => '9999_StudentVisitContactList', 'value'=>
	"SELECT
		...
		count(
			DISTINCT I.".cfg_obo_LO::ID.") AS unique_masters_visited,
		count(*) AS raw_total_visits,
		FROM_UNIXTIME(MIN(T.".cfg_obo_Track::TIME.")) AS first_log,
		FROM_UNIXTIME(MAX(T.".cfg_obo_Track::TIME.")) AS last_log
		FROM 
			".cfg_core_User::TABLE." AS U,
			".cfg_obo_Instance::TABLE." AS I,
			".cfg_obo_Track::TABLE." AS T,
			".cfg_obo_LO::TABLE." AS L
		WHERE 
			L.".cfg_obo_LO::ID." = I.".cfg_obo_LO::ID."
		AND 
			U.".cfg_core_User::ID." = T.".cfg_core_User::ID."
		AND 
			I.".cfg_obo_Instance::ID." = T.".cfg_obo_Instance::ID."
		AND 
			T.".cfg_obo_Track::TYPE." = 'nm_los_tracking_Visited'
		AND
			I.".cfg_obo_LO::ID." IN ($loIDs)
		AND
			T.".cfg_obo_Track::TIME." >= '$startTime'
		AND
			T.".cfg_obo_Track::TIME." <= '$endTime'
		AND 
			U.".cfg_core_User::ID." NOT IN (
				 							SELECT
												DISTINCT ".cfg_core_User::ID."
											FROM
												".cfg_obo_Role::MAP_USER_TABLE." )
		GROUP BY
...
$stats[] = array ('name' => '9999_ListOfAllLibraryUsers', 'value'=>
	"SELECT
		U.".cfg_core_User::EMAIL.", U.".cfg_core_User::FIRST.", U.".cfg_core_User::LAST.", FROM_UNIXTIME(U.".cfg_core_User::LOGIN_TIME.")
	FROM
		".cfg_core_User::TABLE." AS U
	LEFT JOIN
		 ".cfg_obo_Role::MAP_USER_TABLE." AS MR
	ON  U.".cfg_core_User::ID." = MR.".cfg_core_User::ID."
	LEFT JOIN
		".cfg_obo_Role::TABLE." AS R
	ON MR.".cfg_obo_Role::ID." = R.".cfg_obo_Role::ID."
	WHERE
		(	
			R.".cfg_obo_Role::ROLE." = '".cfg_obo_Role::CONTENT_CREATOR."'
		OR
			R.".cfg_obo_Role::ROLE." = '".cfg_obo_Role::LIBRARY_USER."'
		)
	AND
		U.".cfg_core_User::LOGIN_TIME." > 0
		GROUP BY U.".cfg_core_User::EMAIL."
	ORDER BY U.".cfg_core_User::LOGIN_TIME." DESC
...
function getQuestionAnswersByInstance($INSTID, $DBM)
{
	...
	$q = $DBM->query("SELECT *, UNCOMPRESS(".cfg_obo_Track::DATA.") as data FROM ".cfg_obo_Track::TABLE." WHERE (".cfg_obo_Track::TYPE." = 'nm_los_tracking_SubmitQuestion' OR ".cfg_obo_Track::TYPE." = 'nm_los_tracking_StartAttempt' OR ".cfg_obo_Track::TYPE." = 'nm_los_tracking_SubmitMedia')  AND ".cfg_obo_Instance::ID." ='{$INSTID}' ORDER BY ".cfg_core_User::ID.", ".cfg_obo_Track::TIME."");
	$curInst=-1;
	$userAttempts = array();
	while($r = $DBM->fetch_obj($q))
	{
