...
class plg_UCFAuth_SsoHash
{
	...
	public function validateSSOHashFromRequest()
	{
		...
		if(!is_null($sso_req[self::SSO_IP]) && $_SERVER['REMOTE_ADDR'] != $sso_req[self::SSO_IP])
		{
			return false;
		}
 ...
 */
	...
	private function getSsoParametersFromReq()
	{
		...
		if(!$this->checkInput($_REQUEST[self::SSO_USERID], "/[[:alnum:]]{2,15}/"))
		{
			...
			throw new Exception("Invalid user id: " . urlencode($_REQUEST[self::SSO_USERID]));
		}

		if(!$this->checkInput($_REQUEST[self::SSO_TIMESTAMP], "/[[:digit:]]{10,11}/"))
		{
			unset($param[self::SSO_TIMESTAMP]);
			throw new Exception("Invalid timestamp: " . urlencode($_REQUEST[self::SSO_TIMESTAMP]));
		}
		...
		if(!$this->checkInput($_REQUEST[self::SSO_HASH], "/[[:alnum:]]{16,64}/")) {
			...
			throw new Exception("Invalid SSO authentication token: " . urlencode($_REQUEST[self::SSO_HASH]));
		}
	...
	private function validate($params)
	{
		...
		{
			throw new Exception("Invalid timestamp: " . urlencode($params[self::SSO_TIMESTAMP]));
		}

		if(!$this->checkInput($params[self::SSO_HASH], "/[[:alnum:]]{16,64}/"))
		{
			throw new Exception("Invalid SSO authentication token: " . urlencode($params[self::SSO_HASH]));
		}

		$now = time();
		$tstamp = (int) $params[self::SSO_TIMESTAMP];
		if (($now - $tstamp) > $this->sso_timeout)
		{
			return false;
		}
		return $params[self::SSO_HASH] == $this->generateHash($params);
	}
}
