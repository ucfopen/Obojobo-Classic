	...
	function writeAmf3String($d, $raw = false)
	{
		...
		else
		{
			if( !isset($this->storedStrings[$d]))
			{
				...
				//if(strlen($d) < 64)  //NEWMEDIA  if we choose to hide the server object names, they are typically shorter then 65 chars long resulting in exponentially larger responce sets, lets use the string set for just about everything
				if(strlen($d) < 4)
				{
