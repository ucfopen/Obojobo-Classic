...
class MasterTest extends PHPUnit_Framework_TestCase
{
	...
	public function testGetUserName()
	{
		...
		@$this->assertEquals(new nm_los_Error(1), $this->MAS->getUserName(1));
	...
	public function testGetAllUsers()
	{
   	...
   	{
		$this->MAS = new nm_los_Master();
   		@$this->assertEquals(new nm_los_Error(2), $this->MAS->deleteLO());
   		@$this->assertEquals(new nm_los_Error(1), $this->MAS->deleteLO(1));
   		@$this->assertEquals(new nm_los_Error(2), $this->MAS->deleteLO('asdfadsfadsf'));
   		@$this->assertTrue($this->MAS->login(self::ADMIN_USER, md5(self::ADMIN_PW)));
    ...
    public function testGetLOFull()
 	{
		$this->MAS = new nm_los_Master();
 		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOFull());
 		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOFull('afadfs'));
 		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOFull(0));
 		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOFull(-1));
	...
	public function testGetLOMeta()
	{
		...
		$LOID = $newLO->id;
		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOMeta());
		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOMeta('afadfs'));
		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOMeta(0));
		@$this->assertEquals(new nm_los_Error(2), $this->MAS->getLOMeta(-1));
	...
	{
		$this->MAS = new nm_los_Master();
		@$this->assertEquals(new nm_los_Error(1), $this->MAS->getMyObjects());
	...
	{
		// $this->MAS = new nm_los_Master();
		// @$this->assertEquals(new nm_los_Error(1), $this->MAS->getPublicMasters());
		// 
		// @$this->assertTrue($this->MAS->login(self::ADMIN_USER, md5(self::ADMIN_PW)));
   ...
   // 	{
   // 		$INSTID = 309;
   // 		@$this->assertEquals(new nm_los_Error(2), $this->MAS->submitQuestion());
   // 		@$this->assertTrue($this->MAS->login(self::ADMIN_USER, md5(self::ADMIN_PW)));
   ...
   // 		$this->assertEquals(new nm_los_Error(2), $submit);
   // 
   ...
   // 	
	...
	{
		$this->MAS = new nm_los_Master();
		@$this->assertEquals(new nm_los_Error(1), $this->MAS->getAllLatestLODrafts());
   ...
   // 	}
	...
	protected function doTestLO($LOID, $lo, $testMeta=false, $testInstance=false)
	{
		...
		$this->assertTrue(nm_los_Validator::isPosInt($lo->id));
		...
		$this->assertTrue(nm_los_Validator::isPosInt($lo->lang));
		...
		if(!$testMeta) $this->assertTrue(nm_los_Validator::isPosInt($lo->desc->id));
		$this->assertObjectHasAttribute('text', $lo->desc);
		if($isMaster) $this->assertTrue(nm_los_Validator::isString($lo->desc->text));
		...
		if(!$testMeta) $this->assertTrue(nm_los_Validator::isPosInt($lo->objective->id));
		if($isMaster) $this->assertTrue(nm_los_Validator::isString($lo->objective->text));
		$this->assertObjectHasAttribute('learn_time', $lo);

		//$this->assertGreaterThanOrEqual(0, $lo->learn_time);
		$this->assertTrue(nm_los_Validator::isInt($lo->learn_time));
		$this->assertTrue(nm_los_Validator::isInt($lo->vers_part));
		$this->assertTrue(nm_los_Validator::isInt($lo->vers_whole));

		$this->assertObjectHasAttribute('root', $lo);
		$this->assertTrue(nm_los_Validator::isPosInt($lo->root));
		$this->assertObjectHasAttribute('parent', $lo);
		$this->assertTrue(nm_los_Validator::isInt($lo->parent));
		...
		$this->assertTrue(nm_los_Validator::isPosInt($lo->datec));
		...
		if(!$testMeta) 
		{
			...
			foreach($lo->pages AS &$page)
			{
				...
				$this->assertTrue(nm_los_Validator::isPosInt($page->id));
				...
				$this->assertTrue(nm_los_Validator::isInt($page->author));
				$this->assertObjectHasAttribute('layout', $page);
				$this->assertTrue(nm_los_Validator::isPosInt($page->layout));
				$this->assertObjectHasAttribute('datec', $page);
				$this->assertTrue(nm_los_Validator::isPosInt($page->datec));
				...
				foreach($page->items as &$item)
				{
					...
					$this->assertTrue(nm_los_Validator::isPosInt($item->id));
			...
			foreach($lo->pgroup->kids AS &$pQ)
			{
				...
				$this->assertTrue(nm_los_Validator::isPosInt($pQ->id));
				$this->assertObjectHasAttribute('author', $pQ);
				$this->assertTrue(nm_los_Validator::isPosInt($pQ->author));
				...
				foreach($pQ->items as $pQItem)
				{
					...
					$this->assertTrue(nm_los_Validator::isPosInt($pQItem->id));
					...
					if($pQItem->layout_item_id == 'MediaView')
					{
						...
						foreach($pQItem->media as $media)
						{
							...
							$this->assertTrue(nm_los_Validator::isPosInt($media->date_created));
			...
			foreach($lo->layouts AS $layout)
			{
				...
				$this->assertTrue(nm_los_Validator::isPosInt($layout->id));
		...
		$this->assertTrue(nm_los_Validator::isPosInt($lo->summary['content_size']));
		...
		$this->assertTrue(nm_los_Validator::isPosInt($lo->summary['practice_size']));
		$this->assertArrayHasKey('assessment_size', $lo->summary);
		$this->assertTrue(nm_los_Validator::isPosInt($lo->summary['assessment_size']));
	}
