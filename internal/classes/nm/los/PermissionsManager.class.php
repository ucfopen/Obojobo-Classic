 ...
 */
class nm_los_PermissionsManager extends core_db_dbEnabled
{
	...
	private function getPerms($itemID=0, $itemType='l', $optUserID=0)
	{
		...
		if(!nm_los_Validator::isPosInt($optUserID, true))
		{
			$optUserID = 0;
		}
		$itemType = mysql_real_escape_string($itemType);
		// 
		// if($permObj = core_util_Cache::getInstance()->getPerms($itemID, $itemType, $optUserID))
		// {
		// 	return $permObj;
		// }
		...
		{
			$permObj = new nm_los_Permissions(
				$r->{cfg_core_User::ID},
				$r->{cfg_obo_Perm::READ},
			...
			//core_util_Cache::getInstance()->setPerms($itemID, $itemType, $optUserID, $permObj);
	...
	public function getMergedPerms($itemID=0, $itemType='l', $optUserID=0)
	{
		...
		if($optUserID == 0 && nm_los_Validator::isPosInt($_SESSION['userID']) )
		{
			$optUserID = $_SESSION['userID'];
		}
		if(nm_los_Validator::isPosInt($optUserID))
		{
	...
	public function setUsersPerms($permObjects, $itemID = 0, $itemType = 'l')
	{
		...
		{
			$res = $this->setPerms($itemID, $itemType, new nm_los_Permissions($permObj));
	...
	{
		if(! nm_los_Validator::isPosInt($itemID, true) )
		{
		...
		if(!($r = $this->DBM->fetch_obj($q)))
		{
	        ...
	        return core_util_Error::getError(5003);
		}
		...
		//
		//core_util_Cache::getInstance()->clearPerms($itemID, $itemType, $userID);
		return true;
	}
	...
	private function setPerms($itemID=0, $itemType='l', $permObj)
	{
		...
		//core_util_Cache::getInstance()->clearPerms($itemID, $itemType, $permObj->userID);
		return true;
	}
