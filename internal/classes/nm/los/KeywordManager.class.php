 ...
 * 'lay' = layout
 */
class nm_los_KeywordManager extends core_db_dbEnabled
{
	...
	public function keywordExists($keyname='')
	{
		...
		if( $r = $this->DBM->fetch_obj($q) )
		{
			return new nm_los_Keyword($r->{cfg_obo_Keyword::ID}, $r->{cfg_obo_Keyword::NAME});
		}
		else
		{
			return false;
	...
	public function newKeyword($keyname='')
	{
		...
		if( $key == false )
		{
			...
			unset($key);
			$key = new nm_los_Keyword($this->DBM->insertID, $keyname);
		}
		return $key;
	}
